...
EOF
		...
		tail -n 10 /tmp/cloudstomp-$SESSION/plain/output.txt
		if [ -f /tmp/cloudstomp-$SESSION/plain/mysession_cracked ]; then
			echo ""
			echo "Cracked:"
			echo "--------------------"
			cat /tmp/cloudstomp-$SESSION/plain/mysession_cracked
			echo "--------------------"
		else
			echo ""
			echo "No hashes cracked."
		fi
		if [ "${PASSWORD}" != '' ]; then
